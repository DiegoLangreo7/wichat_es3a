import { TemplateModel } from '../model/questionTemplate';
import {
    getRandomItem,
 ...
 */
async function generateQuestions(size: number, types: any = []): Promise<object[] | void> {
    console.log('------------------');
 ...
 */
function performChecks(questionsArray: any[], lang: string = 'es') {
...
/*
  ...
  while(templates.length < n){
    ...
    // Apart from that, this part was adding directly the arrays:
...
const generateQuestionJson = async (
    ...
    try {
        ...
        try {
            ...
            // If an error has occurred no question is generated
            return undefined;
        }
